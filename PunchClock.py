...
class PunchClockPopup:
    ...
    def expand_hour_button(self, hour):
        ...
        for child in self.quarter_frame.winfo_children():
            child.destroy()
        # Center the minute-selection frame over the existing content
        self.quarter_frame.place(relx=0.5, rely=0.5, anchor=CENTER)
        self.quarter_frame.lift()
    ...
    def reset_hour_buttons(self, *_):
        ...
        for btn in self.quarter_buttons:
            btn.destroy()
        self.quarter_buttons.clear()
        self.quarter_frame.place_forget()
