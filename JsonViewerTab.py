...
class JsonViewerTab:
    ...
    def _build_ui(self):
        ...
        self.period_menu = ttk.Combobox(header_frame, textvariable=self.pay_period_var, state="readonly")
        ...
        # Inputs Text Box
        input_frame = ttk.LabelFrame(self.frame, text="Valeurs entrées")
        ...
        columns = ("id", "name", "hours", "cash", "sur_paye", "frais_admin")
        ...
        ttk.Button(button_frame, text="Supprimer cette distribution", bootstyle=DANGER, command=self.delete_selected_file).pack(side=LEFT, padx=5)
    ...
    def delete_selected_file(self):
        ...
        confirm = messagebox.askyesno("Confirmer la suppression", f"Êtes vous sûr de vouloir supprimer '{file_name}' ?")
        if confirm:
            try:
